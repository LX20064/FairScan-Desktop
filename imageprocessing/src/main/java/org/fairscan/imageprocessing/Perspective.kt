...
fun estimateRealDimensions(
    ...
    // 数值病态防护：文档接近正平面时消失点位于无穷远附近，f 估计落入中等区间，
    // 平面重建会产出极小/极大（甚至 NaN）的边长，导致输出比例严重失真。
    // 此时回退到平均边长（保持文档在源图中的真实宽高比）。
    val diag = hypot(imageWidth.toDouble(), imageHeight.toDouble())
    val plausible = realW.isFinite() && realH.isFinite() &&
        realW > diag * 1e-4 && realW < diag * 1e2 &&
        realH > diag * 1e-4 && realH < diag * 1e2
    if (!plausible) return averageSides()
